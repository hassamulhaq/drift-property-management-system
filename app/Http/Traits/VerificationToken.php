...
{
    public function updateOrCreateVerificationToken($paramsData = []): UserVerify
    {
        $userId = $paramsData['user_id'] ?? 0;
        $type = $paramsData['type'] ?? '';
        $typeValue = $paramsData['type_value'] ?? '';
        $verify = UserVerify::where(['user_id' => $userId, 'type' => $type])->firstOrFail();
    ...
    public function generateVerificationToken($digits = 4): string
    {
        $i = 0; //counter
    ...
    public static function customQuickRandom($length = 16): string
    {
